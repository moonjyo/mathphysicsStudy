...
void SoftRenderer::Update2D(float InDeltaSeconds)
{
	...
	CameraObject& goCamera = g.GetMainCamera();
	assert(goPlayer.IsValid());

	// 입력에 따른 플레이어 위치와 크기의 변경
	TransformComponent& cameraTransform = goCamera.GetTransform();
	cameraTransform.AddPosition(Vector2(input.GetAxis(InputAxis::XAxis), input.GetAxis(InputAxis::YAxis)) * moveSpeed * InDeltaSeconds);

	TransformComponent& transform =  goPlayer.GetTransform(); //goCamera.GetTransform();
...
void SoftRenderer::Render2D()
{
	auto& r = GetRenderer();
	const auto& g = Get2DGameEngine();
	const auto& c = g.GetMainCamera();
	Matrix3x3 viewMatrix = c.GetViewMatrix();
